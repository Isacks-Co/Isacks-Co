...
class PostProcessing:
    ...
    def computeLatticeConstant(self) -> float:
        ...
        cell = self.atoms.cell.array  # numpy.ndarray of shape (3, 3)
        ...
        if struct == "fcc":
            ...
            logger.info("a (from primitive-cell volume) = {:.6f} Å".format(a_from_V))
            ...
            logger.info("a (if conventional cell) = {:.6f} Å".format(a_conv))
    ...
    def computeBulkModulus(self, scales = np.linspace(0.96, 1.04, 7)):
        """
        ...
        #logger.info("Bulk modulus = {:.6f} GPa".format(bulk_modulus))
    ...
    def computeInternalPressure(self):
        ...
        logger.info(f"Average internal pressure = {average_internal_pressure} GPa")
    ...
    def computeSelfDiffusionCoefficient(self):
        timestep_list = []
        temp_list = [round(self.data_log["T[K]"][i], -1) for i in range(len(self.data_log["T[K]"]))]
        logger.info(f"Temperature list: {temp_list}")
        ...
        logger.info(f"Timestep list: {timestep_list}")
        ...
        logger.info(f"Self-Diffusion Coefficient------------ [Å^2/s^-1]: {slope_m2_per_s}")
    ...
    def computeDebyeTemperature(self):
        ...
        D3 = []
        for i in range(len(self.data_log["T[K]"])):
            D3.append(234  * (self.data_log["T[K]"][i])**3 / C_v)
        logger.info("Debye temperature ^3: ", D3)
        D3_mean = np.average(D3)
        logger.info("Average Debye temperature ^3: ", D3_mean)
        ...
        for i in range(len(traj)):
            ...
            #logger.info("Debye temperature: ", D)
            debye.append(D)

        debye_avg = np.average(debye)
        logger.info(f"Average Debye temperature: {debye_avg}")
        return debye_avg
